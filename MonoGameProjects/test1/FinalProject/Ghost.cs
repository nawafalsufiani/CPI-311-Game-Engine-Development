...
namespace CPI311.Labs
{
    public class Ghost : GameObject
    {
        ...
        SoundEffect blueGhostColidePacman, normalGhostColidePacman, returnGhostColidePacman;
        SoundEffectInstance soundInstance;



        public Ghost(TerrainRenderer terrain, ContentManager Content,
            Camera camera, GraphicsDevice graphicsDevice, Light light, Player theplayer) : base()
        {
            ...
            normalGhostColidePacman = Content.Load<SoundEffect>("pacman_death");
            blueGhostColidePacman = Content.Load<SoundEffect>("pacman_eatghost");

            returnGhostColidePacman = Content.Load<SoundEffect>("Ghost： Return to Home");
            ...
            Renderer renderer = new Renderer( Content.Load<Model>("ghost"), Transform, camera, Content, graphicsDevice, light,1, "SimpleShading", 20f, texture);


            renderer.Material.Diffuse = Color.Blue.ToVector3();
            renderer.Material.Specular = Color.Blue.ToVector3();
            renderer.Material.Ambienet = Color.Blue.ToVector3();
        ...
        public override void Update()
        {
            ...
            if (IsCollidingWithPlayer() && PowerUp.PowerStatus == false)
            {
                
                
                SoundEffectInstance soundInstance = normalGhostColidePacman.CreateInstance();
                soundInstance.IsLooped = false;
                soundInstance.Play();


                FinalProject.pacmanDeath = true;
                JumpToRandomPosition();
            }

            else if (IsCollidingWithPlayer() && PowerUp.PowerStatus == true)
            {


                FinalProject.currentScore = FinalProject.currentScore + 50;
                SoundEffectInstance soundInstance = blueGhostColidePacman.CreateInstance();
                soundInstance.IsLooped = false;
                soundInstance.Play();

                
                    soundInstance = returnGhostColidePacman.CreateInstance();
                soundInstance.IsLooped = false;
                soundInstance.Play();
